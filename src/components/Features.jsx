...
const features = [
  ...
  { icon: <Mic size={24} />, title: 'Prophetic Voice', desc: 'Declaring God\'s word with boldness and accuracy in this critical hour across every sphere of society.' },
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <h2 className={`font-display text-4xl font-bold mb-4 ${dark ? 'text-white' : 'text-gray-900'}`}>
            Our Ministry <span className="gold-text">Pillars</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((f, i) => (
            <div key={i}
              className={`program-card p-7 rounded-2xl group
                ${dark
                  ? 'bg-white/4 border border-white/8 hover:border-yellow-600/30 hover:bg-white/6'
                  : 'bg-white border border-gray-100 hover:border-yellow-500/40 shadow-sm hover:shadow-md'
                }`}>
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-yellow-500/20 to-amber-600/20 flex items-center justify-center text-yellow-500 mb-5 group-hover:scale-110 transition-transform">
                {f.icon}
              </div>
