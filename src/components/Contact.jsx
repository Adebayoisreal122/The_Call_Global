...
export default function Contact() {
  const { dark } = useTheme();
  const { submitPrayer } = useMinistry();
  ...
  const submitPrayerForm = async (e) => {
    ...
    try {
      await submitPrayer(pForm);
      setPDone(true);
      setPForm({ name: '', email: '', request: '' });
      setTimeout(() => setPDone(false), 4000);
    } catch (err) {
  ...
  return (
    <section id="contact" className={`py-24 ${dark ? 'bg-[#04093a]' : 'bg-white'}`}>
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <h2 className={`font-display text-4xl font-bold mb-4 ${dark ? 'text-white' : 'text-gray-900'}`}>
            Reach <span className="brand-text">Out To Us</span>
          </h2>
          ...
          <div className="space-y-6">
            ...
            <div className={`mt-8 p-5 rounded-2xl border border-blue-600/20 ${dark ? 'bg-blue-500/5' : 'bg-blue-50'}`}>
              ...
              <p className={`text-sm font-body ${dark ? 'text-gray-400' : 'text-gray-600'}`}>
                For pastoral counseling, ministry partnership, or speaking invitations, reach out and we'll schedule a session.
              </p>
            </div>
          </div>
            ...
            {tab === 'prayer' ? (
              pDone ? (
                <div className="text-center py-12">
                  ...
                  <p className={`text-sm mt-2 ${dark ? 'text-gray-400' : 'text-gray-600'}`}>Your request has been received. Our prayer team will stand with you.</p>
                </div>
              ) : (
                <form onSubmit={submitPrayerForm} className="space-y-4">
                  {pError && <p className="text-red-400 text-sm">{pError}</p>}
                  <input value={pForm.name} onChange={(e) => setPForm((f) => ({ ...f, name: e.target.value }))} placeholder="Your Name" className={input} />
                  <input value={pForm.email} onChange={(e) => setPForm((f) => ({ ...f, email: e.target.value }))} type="email" placeholder="Email Address" className={input} />
                  <textarea value={pForm.request} onChange={(e) => setPForm((f) => ({ ...f, request: e.target.value }))}
                    placeholder="Share your prayer request... *" required rows={5} className={`${input} resize-none`} />
                  <button type="submit" disabled={pSaving}
                    ...
                    {pSaving ? <><Loader2 size={15} className="animate-spin" /> Sending...</> : <><Send size={15} /> Send Prayer Request</>}
                  </button>
                </form>
              )
            ) : (
              cDone ? (
                ...
                <form onSubmit={submitContact} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <input value={cForm.name} onChange={(e) => setCForm((f) => ({ ...f, name: e.target.value }))} placeholder="Your Name *" required className={input} />
                    <input value={cForm.email} onChange={(e) => setCForm((f) => ({ ...f, email: e.target.value }))} type="email" placeholder="Email *" required className={input} />
                  </div>
                  <input value={cForm.subject} onChange={(e) => setCForm((f) => ({ ...f, subject: e.target.value }))} placeholder="Subject" className={input} />
                  <textarea value={cForm.message} onChange={(e) => setCForm((f) => ({ ...f, message: e.target.value }))} placeholder="Your message..." required rows={5} className={`${input} resize-none`} />
