...
export default function Programs() {
  ...
  return (
    <section
      id="programs"
      className={`py-24 ${dark ? "bg-gray-950" : "bg-blue-950"}`}
    >
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="font-body text-lg max-w-xl mx-auto text-blue-200/70">
            Don't miss what the Spirit is doing. Join us at these upcoming
            ministry events.
          </p>
        </div>
        ...
        {upcoming.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {upcoming.map((p, index) => (
              <div
                key={p.id || index}
                className="program-card rounded-2xl overflow-hidden group bg-white/6 border border-white/10 hover:border-blue-400/30 hover:bg-white/10"
              >
                <div className="relative h-40 overflow-hidden">
                  <img
                    src={p.image || "/logowhite.png"}
                    alt={p.title}
                  ...
                  {/* Type badge */}
                  <div
                    className={`absolute top-3 left-3 inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${typeBadge[p.type] || "text-blue-300 bg-blue-500/15"}`}
                  >
                    {p.type}
                  </div>
                </div>
                <div className="p-6">
                  <h3 className="font-display text-lg font-semibold mb-2 text-white group-hover:text-blue-200 transition-colors">
                    {p.title}
                  </h3>
                  <p className="font-body text-sm leading-relaxed mb-4 text-blue-200/70">
                    {p.description}
                  </p>
                  <div className="space-y-2 text-xs text-blue-300/60">
                    ...
                    <div className="flex items-center gap-2">
                      <Clock size={13} className="text-blue-400" /> {p.time}
                    </div>
                    <div className="flex items-center gap-2">
